...
def plot_xyY_color_volume_sdr_hdr(
    ...
    # fname = "~/abuse/2020/023_color_volume/img_seq/"\
    #     + f"xyY_SDR_HDR_angle_{f_idx:04d}.png"
    print(fname)
    plt.savefig(
        fname, bbox_inches='tight', pad_inches=0.1)
...
def plot_xyY_color_volume_pyqtgraph(
    ...
    # w = gl.GLViewWidget()
    w = myGLViewWidget(width=1200, height=960)
    w.opts['distance'] = 1.7
    w.opts['center'] = Vector(cs.D65[0], cs.D65[1], 0.5)
    w.opts['elevation'] = 30
    w.opts['azimuth'] = -120
    # w.opts['viewport'] = [0, 0, 1024, 768]
    w.show()
...
def plot_reduced_sample_plotly(
        xyY_reduced_data, alpha=1.0, color_space_name=cs.BT2020,
        min_z=0.0, max_z=1.0):
    out_buf_xyz = []
    out_buf_rgb = []
    ...
    out_buf_rgb = np.vstack(out_buf_rgb)
    # size_val = np.ones(out_buf_xyz.shape[0]) * size
    df_xyz = pd.DataFrame(data=out_buf_xyz, columns=['x', 'y', 'Y'])
    df_rgb = pd.DataFrame(data=out_buf_rgb, columns=['x', 'y', 'Y'])
    fig = px.scatter_3d(
        df_xyz, x='x', y='y', z='Y', color=df_rgb,
        width=1024, height=1024)

    fig.show()


def plot_xyY_color_volume_sdr_hdr_plotly(
        f_idx, xyY_data_hdr, xyY_reduced_data_hdr,
        xyY_data_sdr, xyY_reduced_data_sdr, y_step=1,
        rad_rate=4.0, angle=-120, line_div=40, color_space_name=cs.BT2020,
        min_exposure_sdr=-4, max_exposure_sdr=0,
        min_exposure_hdr=-8, max_exposure_hdr=0):
    plot_reduced_sample_plotly(
        xyY_reduced_data=xyY_reduced_data_hdr, alpha=1.0)
