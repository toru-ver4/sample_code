...
def create_curve_type_element(root):
    """
    Create a curveType element for rTRC/gTRC/bTRC and return its Curve element.

    If the profile already has a parametricCurveType or curveType element for
    the RGB TRC tags, it is replaced in-place.
    """
    tags_element = root.find("Tags")
    if tags_element is None:
        tags_element = root

    trc_tag_list = ["rTRC", "gTRC", "bTRC"]
    curve_type_element = ET.Element("curveType")
    curve_type_element.text = "\n      "

    for trc_tag in trc_tag_list:
        tag_signature = ET.SubElement(curve_type_element, "TagSignature")
        tag_signature.text = trc_tag
        tag_signature.tail = "\n      "

    curve_element = ET.SubElement(curve_type_element, "Curve")
    curve_element.tail = "\n    "

    target_idx = None
    for idx, child in enumerate(list(tags_element)):
        if child.tag not in ["parametricCurveType", "curveType"]:
            continue
        signature_list = [
            element.text for element in child.findall("TagSignature")]
        if all(trc_tag in signature_list for trc_tag in trc_tag_list):
            target_idx = idx
            curve_type_element.tail = child.tail
            break

    if target_idx is not None:
        tags_element.remove(tags_element[target_idx])
        tags_element.insert(target_idx, curve_type_element)
    else:
        curve_type_element.tail = "\n  "
        tags_element.append(curve_type_element)
...
def set_curve_type_params_to_element(lut, curve_element):
    """
    Set a curveType Curve element from a common RGB uint16 LUT.

    Parameters
    ----------
    lut : ndarray(np.uint16)
        A one-dimensional LUT. The number of entries must be a power of two.
    curve_element : xml.etree.ElementTree.Element
        An instance of the Element class. It indicates the Curve tag.
    """
    lut = np.asarray(lut)
    if lut.ndim != 1:
        raise ValueError("lut must be a one-dimensional array")

    num_of_entries = len(lut)
    if num_of_entries < 2:
        raise ValueError("lut must have at least 2 entries")
    if num_of_entries & (num_of_entries - 1):
        raise ValueError("the number of lut entries must be a power of two")
    ...
    head_space = " " * 8
    out_buf = "\n"
    for st_idx in range(0, num_of_entries, element_per_line):
        ed_idx = st_idx + element_per_line
        line = [f"{x:5d}" for x in lut[st_idx:ed_idx]]
        out_buf += head_space + " ".join(line) + "\n"
    out_buf += " " * 6

    curve_element.text = out_buf
...
def mhc2_raw_text_to_ndarray(raw_data):
    text_data = raw_data.replace("\n", "").replace(" ", "")
    text_array = re.split('(........)', text_data)[1::2]

    data_array = [int(x, 16) for x in text_array]
    data_array = np.int32(data_array)

    return data_array


def mhc2_ndarray_to_text(data_array):
    text_array = [f"{x:08x}" for x in data_array]
    element_per_line = 8
    num_of_line = int(ceil(len(text_array) / element_per_line))
    out_buf = "\n"
    for l_idx in range(num_of_line):
        out_buf += " " * 7
        st_idx = l_idx * element_per_line

        if l_idx < num_of_line - 1:
            ed_idx = st_idx + element_per_line
            out_buf += "".join(text_array[st_idx:ed_idx])
            out_buf += "\n"
        else:
            out_buf += "".join(text_array[st_idx:])

    return out_buf


def set_mhc2_basic_info(
        out_buf, num_of_1dlut_entries, min_luminance, peak_luminance,
        num_of_basic_info, num_of_matrix_element, elem_size_to_byte):
    # 4 to 7, Reserved
    out_buf[0] = 0x00
    # 8 to 11, Number of 1DLUT entries
    out_buf[1] = num_of_1dlut_entries
    # 12 to 15, ST 2086 min luminance in nits
    out_buf[2] = np.int32(np.round(min_luminance * 0x10000))
    # 16 to 19, ST 2086 peak luminance in nits
    out_buf[3] = np.int32(np.round(peak_luminance * 0x10000))
    # 20 to 23, Offset in bytes to matrix. "+1" means "MHC2" Signature
    out_buf[4] = (num_of_basic_info + 1) * elem_size_to_byte
    # 24 to 27, Offset in bytes to red 1DLUT
    out_buf[5] = out_buf[4] + num_of_matrix_element * elem_size_to_byte
    # 28 to 31, Offset in bytes to green 1DLUT
    out_buf[6] = out_buf[5] + (num_of_1dlut_entries + 2) * elem_size_to_byte
    # 32 to 35, Offset in bytes to blue 1DLUT
    out_buf[7] = out_buf[6] + (num_of_1dlut_entries + 2) * elem_size_to_byte


def set_mhc2_matrix_params(out_buf, mtx, elem_size_to_byte):
    """
    Parameters
    ----------
    out_buf : ndarray(np.int32)
        output buffer
    mtx : ndarray(float32)
        A matrix. Shape is (3, 3)
    elem_size_to_byte : int
        4
    """
    matrix_st_idx = (out_buf[4] // elem_size_to_byte) - 1
    for c_idx in range(3):  # "3" means R, G, B colors
        st_idx = c_idx * 4 + matrix_st_idx
        out_buf[st_idx+0] = np.int32(np.round(mtx[c_idx, 0] * 0x10000))
        out_buf[st_idx+1] = np.int32(np.round(mtx[c_idx, 1] * 0x10000))
        out_buf[st_idx+2] = np.int32(np.round(mtx[c_idx, 2] * 0x10000))


def set_mhc2_1dlut_params(out_buf, luts, elem_size_to_byte):
    """
    Parameters
    ----------
    out_buf : ndarray(np.int32)
        output buffer
    luts : ndarray(float32)
        1DLUTs. Shape is (N, 3)
    elem_size_to_byte : int
        4
    """
    luts_int32 = np.int32(np.round(luts * 0x10000))
    data_len = len(luts)
    for c_idx in range(3):  # "3" means R, G, B colors
...
def set_mhc2_params_to_element(
        mhc2_element, min_luminance, peak_luminance, matrix, luts):
    """
    Parameters
    ----------
    mhc2_element : xml.etree.ElementTree.Element
        A elemnent of the MHC2 tag.
    min_luminance : float
        Minimum luminance of the display
    peak_luminance : float
        Peak luminance of the display
    matrix : ndarray(float)
        Matrix for the calibration (XYZ space)
        Shape is (3, 3).
    luts : ndarray(float)
        1DLUTs for the calibration (after display OETF)
        Shape is (N, 3)
    """
    num_of_1dlut_entries = len(luts)
    elem_size_to_byte = 4
    num_of_basic_info = 8
    num_of_matrix_element = 12
    num_of_total_mhc2_element\
        = num_of_basic_info + num_of_matrix_element\
    ...
    set_mhc2_basic_info(
        out_buf=out_buf, num_of_1dlut_entries=num_of_1dlut_entries,
        min_luminance=min_luminance, peak_luminance=peak_luminance,
        num_of_basic_info=num_of_basic_info,
        num_of_matrix_element=num_of_matrix_element,
        elem_size_to_byte=elem_size_to_byte)
    set_mhc2_matrix_params(
        out_buf=out_buf, mtx=matrix, elem_size_to_byte=elem_size_to_byte)
    set_mhc2_1dlut_params(
        out_buf=out_buf, luts=luts, elem_size_to_byte=elem_size_to_byte)

    # for idx, val in enumerate(out_buf):
    #     print(f"{idx}, {data[idx]:08X}, {val:08X}")
    #     # if idx > 100:
    #         # break
