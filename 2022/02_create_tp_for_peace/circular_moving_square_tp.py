...
class CircularMovingSquareTP():
    def __init__(self, width=1920, height=1080):
        ...
        self.square_size = int(self.active_height * 0.1)
        self.circular_amp = int(self.active_height * 0.25)
    ...
    def create_square_obj_list(self):
        tb_margine = int(self.active_height * 0.085 + 0.5)
        center_active = self.active_height - 2*tb_margine - 2*self.circular_amp
        space_v = center_active / (len(self.square_color_list) - 1)
        st_pos_h = int(self.square_size * 1.5)
        ed_pos_h = self.width - int(self.square_size * 1.5)

        self.square_obj_list: List[CircularSquare] = []
        for idx in range(self.square_num):
            st_pos_v = int(
                tb_margine + self.circular_amp + space_v * idx + 0.5)
            accel_rate = self.accel_rate_list[idx]
            square_obj = CircularSquare(
                ...
                h_len=ed_pos_h-st_pos_h, amp_v=self.circular_amp,
...
class CircularSquare():
    ...
    def calc_pos_h(self, idx):
        y_h = np.sin(
            ...
            * 0.5 + 0.5
        y_v = np.sin(2*np.pi/(self.num_of_frame)*idx*4)
        pos_h = int(y_h * self.h_len + 0.5) - (self.size // 2)
        pos_v = int(y_v * self.amp_v + 0.5) - (self.size // 2)
        self.st_pos = [
            self.global_st_pos[0] + pos_h,
            self.global_st_pos[1] + pos_v]
        self.ed_pos = [self.st_pos[0] + self.size, self.st_pos[1] + self.size]

        self.debug_y.append(y_h)
...
def main_func():
    moving_square_tp = CircularMovingSquareTP(width=1920, height=1080)
    moving_square_tp.draw_square_seq()
    # moving_square_tp = CircularMovingSquareTP(width=3840, height=2160)
    # moving_square_tp.draw_square_seq()
