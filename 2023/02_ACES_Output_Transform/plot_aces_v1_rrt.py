...
def convert_aces_ap0_to_bt709_using_aces_ot(grid_num):
    ...
    ocio_path = "/work/src/2023/02_ACES_Output_Transform/ty_config.ocio"
    cmd = [
        'ocioconvert', '--view', in_fname, "ACES2065-1",
        out_fname, "BT709-BT1886", "ACES_OT - SDR-VIDEO_BT709_1.1"]
...
def plot_aces_v1_rrt_core(grid_num):
    img_fname = create_aces_ap0_to_bt709_envelope_fname(grid_num)
    rgb_bt709 = read_image(img_fname)
    rgb_bt709_linear = tf.eotf(rgb_bt709, tf.GAMMA24)
    large_xyz_bt709 = cs.rgb_to_large_xyz(
        rgb=rgb_bt709_linear, color_space_name=cs.ACES_AP0)
    ...
    # mesh
    x, y, Y = tsplit(xyY_bt709)
    color_rgb = np.clip(rgb_bt709.reshape(-1, 3), 0.0, 1.0)
    ax.scatter3D(x, y, Y, color=color_rgb, s=30)
