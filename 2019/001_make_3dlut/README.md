...
結果を以下に示す。サイズが大きいためオリジナルは Github に「後でファイル名を決定」として置いた。
...
|8 | 128 | 0 | 0 | 149 | 0 | 0|
| 中略 |  |  |  |  |  |  |
|73450 | 0 | 400 | 272 | 0 | 645 | 273|
|73451 | 16 | 400 | 272 | 0 | 645 | 273|
|73452 | 32 | 400 | 272 | 0 | 645 | 273|
|73453 | 48 | 400 | 272 | 0 | 645 | 273|
|73454 | 64 | 400 | 272 | 0 | 645 | 273|
|73455 | 80 | 400 | 272 | 0 | 645 | 273|
|73456 | 96 | 400 | 272 | 0 | 645 | 273|
|73457 | 112 | 400 | 272 | 0 | 645 | 273|
|73458 | 128 | 400 | 272 | 0 | 645 | 273|
| 中略 |  |  |  |  |  |  |
...
3DLUT の格子点数は 17x17x17x, 33x33x33, 65x65x65 など **2のべき乗+1** の値を使うケースが多い。**+1** をすることで格子点の幅が2のべき乗に近づくのが理由だと筆者は考えている。

具体例を以下に示す。17x17x17 の場合と 16x16x16 の場合の格子点の値を 10bit の整数型で表示すると以下となる。
...
ここからは実際の Pythonコードの説明となる。まず筆者が作成したサンプルコードを示す。
...
念のため 2. について補足しておく。```make_3dlut_grid``` の出力の実例を以下に示す。
...
R, G, B の順にデータが増加していることが分かると思う。なお、行数の関係で ```grid_num=3``` としている。この順序にした理由は後述する 3DLUTフォーマットの1つである Cube 形式の増加順序に合わせたためである。
...
LUT値が完ししたら Davinci Resolve 等の各種ツールで読み込めるよう Adobe の Cube 形式[4]でファイルに保存する。Cube 形式を採用したのは広く使われているからである。例えば SONY の S-Log3/S-Gamut3 や キヤノンの CinemaGamut/CanonLog3 を変換する 3DLUT は Cube 形式て提供されている[5][6]。
